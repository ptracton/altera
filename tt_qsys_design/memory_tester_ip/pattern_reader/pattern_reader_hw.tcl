...
# +-----------------------------------
# | module pattern_reader
# | 
set_module_property DESCRIPTION "Command-based read master that outputs streaming data"
set_module_property NAME pattern_reader
set_module_property VERSION 1.1
set_module_property GROUP "Memory Test Microcores"
set_module_property AUTHOR JCJB
set_module_property DISPLAY_NAME "Pattern Reader"
set_module_property TOP_LEVEL_HDL_FILE mtm_pattern_reader.v
set_module_property TOP_LEVEL_HDL_MODULE mtm_pattern_reader
...
# | files
# | 
add_file mtm_pattern_reader.v {SYNTHESIS SIMULATION}
# | 
# +-----------------------------------
...
add_parameter LENGTH_WIDTH INTEGER 21 "Width of the length register, reduce the value to increase the operating frequency of the read master"
...
add_parameter FIFO_DEPTH INTEGER 128 "The internal FIFO is used to buffer read data between the read master and streaming source port.  Make sure the depth is large enough for the read latency of the slave port the master is connected to as well as at least twice the maximum burst size."
...
add_parameter BURST_ENABLE INTEGER 0 "Enable bursting when connecting the read master to burst capable slave ports like SDRAM for example"
...
add_parameter BURST_REALIGN_ENABLE INTEGER 1 "Enable burst re-alignment support if you want the read master to progress to the next burst boundary before performing full bursts.  This will improve throughput if you connect the master to a burst wrapping slave such as SDRAM"
...
set_interface_property command dataBitsPerSymbol 97
...
add_interface_port command snk_command_data data Input 97
...
# | 
add_interface st_data avalon_streaming start
...
add_interface_port st_data src_data data Output DATA_WIDTH
add_interface_port st_data src_valid valid Output 1
add_interface_port st_data src_ready ready Input 1
# | 
# +-----------------------------------
...
add_interface_port mm_data master_read read Output 1
add_interface_port mm_data master_readdata readdata Input DATA_WIDTH
add_interface_port mm_data master_readdatavalid readdatavalid Input 1
...
proc validate_me {}  {
  ...
  if { ([get_parameter_value BURST_ENABLE] == 1) && ([get_parameter_value FIFO_DEPTH] < (4 * [get_parameter_value MAX_BURST_COUNT])) }  {
    send_message Error "You must set the FIFO depth to be at least four times the maximum burst count."
  }
